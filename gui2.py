...
class Gui():
  ...
  def process(self):
    """ do the processing """
    files = reduce(lambda x,y: x+y, [[os.path.join(i[0],"123.txt") for i in filter(lambda x:
    '123.txt' in x[2],[i for i in os.walk(d)])] 
      for d in self.lb.get(0,END)],[])
    angles.process(self.outfile.get(),files,dir=True)
